...
# Load data in
guide_key <- fread("guide_key.tsv")

tags_to_genes <- fread("tags_to_genes.tsv")

# for example, here we're only going to look at nuo genes
genes_of_interest <- tags_to_genes %>%
  filter(gene %like% "nuo") %>%
  pull(gene)

conditions_of_interest <- c(
  # List of conditions that make sense to analyze, e.g., all versus T0 without induction.
  "None_0_T1 - None_0_T0",
  "None_0_T2 - None_0_T0",
...
max_y_pred <- guide_key %>%
  select(y_pred) %>%
  summarize(max(y_pred, na.rm = TRUE)) %>%
  as.numeric()

# Read results from edgeR
edgeR_results <- fread("edgeR_results.tsv", sep = "\t")

# Annotated edgeR results with "guide_key" and "tags_to_genes" tables
edgeR_results <- edgeR_results %>%
  inner_join(guide_key) %>%
  inner_join(tags_to_genes)
...
mismatches <- edgeR_results %>%
  filter(y_pred > 0) %>%
  filter(condition %in% conditions_of_interest) %>%
  ...
  inner_join(
    edgeR_results %>%
      ...
      select(gene, condition, LFC.adj) %>%
      filter(abs(LFC.adj) == max(abs(LFC.adj))) %>%
      rename(response.max = LFC.adj)
  ) %>%
  nest(data = c(-condition, -gene, -response.max))
...
if (exists("BC.5_model")) {
  ...
  if (check_files_exist(file_names_full)) {
    ...
    if (!is.list(BC.5_model)) stop("Could not load BC.5_model. Exiting script.")
    message("BC.5_model object validated, continuing...")
  } else {
    ...
    BC.5_model_comparison <- BC.5_model %>%
      ...
      select(gene, condition, logLik_model_1, logLik_model_2, better_model)
...
if (exists("BC.5_reduced_model")) {
  ...
  if (check_files_exist(file_names_reduced)) {
    ...
    if (!is.list(BC.5_reduced_model)) stop("Could not load BC.5_reduced_model. Exiting script.")
    message("BC.5_reduced_model object validated, continuing...")
  } else {
    ...
    BC.5_reduced_model_comparison <- BC.5_reduced_model %>%
      ...
      select(gene, condition, logLik_model_3, logLik_model_4, better_model)
...
if (exists("full_results")) {
  ...
  if (!is.list(full_results)) stop("Could not load full_results. Exiting script.")
  message("full_results object validated, continuing...")
} else {
  message("Processing BC.5_model...")
  BC.5_model_processed <- process_mismatches(BC.5_model)
  drc_fits <- BC.5_model_processed %>% select(gene, condition, fit)
...
if (exists("reduced_results")) {
  ...
  if (!is.list(reduced_results)) stop("Could not load reduced_results. Exiting script.")
  message("reduced_results object validated, continuing...")
} else {
  message("Processing BC.5_reduced_model...\n")
  BC.5_reduced_processed <- process_mismatches(BC.5_reduced_model)
  drc_fits <- BC.5_reduced_processed %>% select(gene, condition, fit)
...
if (!is.data.frame(model_comparisons)) stop("Model comparisons not loaded properly. Exiting script.")
message("Model comparisons validated, continuing...")
...
response_data <- full_results$fit_predictions %>%
  ...
  rename(gene = Gene, condition = Condition, fitted = .fitted) %>%
  filter(y_pred != 0) %>%
  group_by(gene, condition) %>%
  summarize(
...
full_estimates <- full_results$model_parameters %>%
  data.table() %>%
  dcast(gene + condition ~ term, value.var = "estimate")
